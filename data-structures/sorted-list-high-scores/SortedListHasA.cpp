...
int SortedListHasA<ItemType>::getPosition(const ItemType& anEntry) const
{
	...
	// Traverse the list to find anEntry or where it belongs
	for (int position = 1; position <= length; position++)
	{
		ItemType currentEntry = listPtr->getEntry(position);

		// If equal, return position
		if (currentEntry == anEntry)
		{
			return position;
		}

		// Since list is highest -> lowest
		// If new entry is larger, it should go BEFORE currentEntry
		else if (anEntry > currentEntry)
		{
			return -position;
		}
	}

	// If it's smaller than all entries, it goes at the end
	return -(length + 1);
}
